import {
  ...
  export const CookieBanner = () => {
    // show a dark overlay on the page with messaging at the botton that says
    // "This site uses cookies to improve your experience. By using this site, you agree to our use of cookies.". The cookie is only for analytics
    // and a button that says "Accept" that closes the banner
    // if accept set local storage variable "lida_consent" to true
    // if local storage variable "lida_consent" is true, do not show the banner
    // if local storage variable "lida_consent" is false, show the banner
  
    const hasConsent = localStorage.getItem("lida_consent");
    const [open, setOpen] = React.useState(!hasConsent);
    const onCloseClick = () => {
      setOpen(false);
      localStorage.setItem("lida_consent", "true");
    };
    return (
      <div className={`bg-secondary rounded py-3 px-4 ${open ? "" : "hidden"}`}>
        <div className="flex gap-3">
          <div className="flex-1 mt-1 text-sm sm:px-0 xl:px-4 ">
            {" "}
            <InformationCircleIcon className="w-4 mr-2  text-accent inline-block" />
            This site uses cookies to improve your experience. By using this site,
            you agree to our use of cookies.
          </div>
          <div onClick={onCloseClick} role="button" className=" ">
            <XMarkIcon className="w-8 duration-300 hover:text-accent  rounded p-1  inline-block" />
          </div>
        </div>
      </div>
    );
  ...
  export const UpdatesBanner = ({ children }: IProps) => {
