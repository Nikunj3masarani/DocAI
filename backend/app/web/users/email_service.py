...
class Users:
    ...
    def invite_user_for_onboarding(self, data):
        try:
            ...
            url = f'''{str(os.getenv("FRONTEND_SERVICE",""))}/set-password?user_uuid={data.get("user_uuid")}&token={data.get("token")}&action={data.get("action")}'''
    ...
    def forget_password(self, data):
        try:
            ...
            url = f'''{str(os.getenv("FRONTEND_SERVICE",""))}/auth/reset-password?user_uuid={data.get("user_uuid")}&token={data.get("token")}&action={data.get("action")}'''
