...
class Users:
    ...
    def invite_user_for_index(self, data):
        ...
        url = str(os.getenv("FRONTEND_SERVICE",""))+"/index-invite?user_uuid="+ data.get("user_uuid")+"&token="+data.get("token")+"&status="+data.get("status")+"&index_uuid="+data.get("index_uuid")
